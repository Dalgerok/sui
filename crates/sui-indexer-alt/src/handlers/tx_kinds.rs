...
mod tests {
    ...
    async fn test_tx_kinds_pruning_complains_if_no_mapping() {
        ...
        let tx_kinds = TxKinds::default();

        let result = tx_kinds.prune(0, 2, &mut conn).await;
    ...
    async fn test_tx_kinds_pruning() {
        ...
        let values = tx_kinds.process(&checkpoint).unwrap();
        ...
        let rows_pruned = tx_kinds.prune(0, 2, &mut conn).await.unwrap();
