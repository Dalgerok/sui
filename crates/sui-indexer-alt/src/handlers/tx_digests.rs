...
mod tests {
    ...
    async fn test_tx_digests_pruning_complains_if_no_mapping() {
        ...
        let tx_digests = TxDigests::default();

        let result = tx_digests.prune(0, 2, &mut conn).await;
    ...
    async fn test_tx_digests_pruning() {
        ...
        let values = tx_digests.process(&checkpoint).unwrap();
        ...
        let rows_pruned = tx_digests.prune(0, 2, &mut conn).await.unwrap();
