...
mod tests {
    ...
    async fn test_tx_affected_objects_pruning_complains_if_no_mapping() {
        ...
        let tx_affected_objects = TxAffectedObjects::default();

        let result = tx_affected_objects.prune(0, 2, &mut conn).await;
    ...
    async fn test_tx_affected_objects_pruning() {
        ...
        let values = tx_affected_objects.process(&checkpoint).unwrap();
        ...
        let rows_pruned = tx_affected_objects.prune(0, 2, &mut conn).await.unwrap();
