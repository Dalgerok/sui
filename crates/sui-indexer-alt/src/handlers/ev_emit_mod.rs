...
mod tests {
    ...
    async fn get_all_ev_emit_mod(conn: &mut db::Connection<'_>) -> Result<Vec<StoredEvEmitMod>> {
        let query = ev_emit_mod::table.load(conn).await?;
        Ok(query)
    }
    ...
    async fn test_ev_emit_mod_pruning_complains_if_no_mapping() {
        ...
        let result = ev_emit_mod.prune(0, 2, &mut conn).await;
    ...
    async fn test_ev_emit_mod_no_events() {
        ...
        let values = ev_emit_mod.process(&checkpoint).unwrap();
    ...
    async fn test_ev_emit_mod_prune_events() {
        ...
        // Test pruning
        let rows_pruned = ev_emit_mod.prune(0, 2, &mut conn).await.unwrap();
